...
# The 'point_connections' array defines connections between vertices.
# Use the index of the vertices from the 'points' list.
# Note: Indexing starts from 1 for better understandability.
point_connections = [[1, 2], [1, 3], [2, 4], [3, 4], [1, 5], [2, 6], [3, 7], [4, 8], [5, 6], [5, 7], [6, 8], [7, 8]]
...
def make_projection_matrix(aspect_ratio, fov, far, near):
    focal_length = 1 / math.tan(math.radians(0.25*fov))
...
while not done:
    ...
    if draw_vertices == True:
        for point in calculated_points:
            pos = (int(point[0] * aspect_ratio * 0.5 * window_size[0] + 0.5 * window_size[0]),
                int(point[1] * 0.5 * window_size[0] + 0.5 * window_size[1]))
            if point[2] == 1:
    ...
    if draw_edges == True:
        for x in point_connections:
            pos1 = (calculated_points[x[0]-1][0] * aspect_ratio * 0.5 * window_size[0] + 0.5 *
                    window_size[0], calculated_points[x[0]-1][1] * 0.5 * window_size[0] + 0.5 * window_size[1])
            pos2 = (calculated_points[x[1]-1][0] * aspect_ratio * 0.5 * window_size[0] + 0.5 *
                    window_size[0], calculated_points[x[1]-1][1] * 0.5 * window_size[0] + 0.5 * window_size[1])
